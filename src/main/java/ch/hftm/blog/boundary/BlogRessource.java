...
public class BlogRessource {
    ...
    @GET
    @PermitAll
    public Response getBlogs(@QueryParam("searchString") Optional<String> searchString, @QueryParam("page") Optional<Long> page) {
    ...
    @DELETE
    public Response deleteBlog(@HeaderParam("blogId") Long blogId) {
        ...
        if (deletedBlog == null) {
            return Response.status(Status.NOT_FOUND).entity("Action not performed. Blog with id " + blogId + " was not found.").build();
        }
    ...
    @PUT
    public Response putBlog(@HeaderParam("blogId") Long blogId,@Valid BlogDTO blog) {
        ...
        BlogDTO replacedBlog = blogService.replaceBlog(blogId, blog);
    ...
    @PATCH
    public Response patchBlog(@HeaderParam("blogId") Long blogId,@Valid BlogDTO blog) {
