...
public class ServiceResponse<T> {
    ...
    // ✅ Make it return Uni<Response> to support reactive execution
    public Uni<Response> createHttpResponse() {
        return Uni.createFrom().item(() -> {
            ...
            if (this.data == null) {
                return switch (this.requestType) {
                    case POST -> Response.status(Response.Status.CONFLICT).entity("Data was not persisted.").build();
                    default -> Response.status(Response.Status.NOT_FOUND).build();
                };
            }

            if (this.data instanceof Iterable<?> iterable && !iterable.iterator().hasNext()) {
                return Response.status(Response.Status.NO_CONTENT).build();
            }

            return switch (this.requestType) {
                case POST -> Response.status(Response.Status.CREATED).entity(this.data).build();
                case DELETE -> Response.status(Response.Status.NO_CONTENT).build();
                default -> Response.status(Response.Status.OK).entity(this.data).build();
            };
        });
    }

    // ✅ Make it return Uni<Response> to support reactive execution
    public Uni<Response> createHttpResponse(UriInfo uriInfo) {
        return Uni.createFrom().item(() -> {
            if (this.exception != null) {
                if (this.errorDto == null) {
                    ErrorDTO error = new ErrorDTO();
            ...
            if (this.requestType == RequestType.POST) {
                if (this.data instanceof BlogPostDTO blog) {
                ...
                } else if (this.data instanceof CommentDTO comment) {
